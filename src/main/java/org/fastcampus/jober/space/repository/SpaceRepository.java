...
//    Optional<Space> findBySpaceName(String spaceName);
    ...
    @Query("SELECT new org.fastcampus.jober.space.dto.response.SpaceListResponseDto(s.spaceId, s.spaceName, sm.authority) FROM Space s LEFT JOIN s.spaceMembers sm WHERE sm.user.userId = :userId")
