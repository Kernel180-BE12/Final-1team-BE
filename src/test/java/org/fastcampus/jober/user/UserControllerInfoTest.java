...
class UserControllerInfoTest {
    ...
    void updateUserInfo_username_success() throws Exception {
        // given
        var updateBody = new UpdateRequestDto("newusername", null, null);
    ...
    void updateUserInfo_name_success() throws Exception {
        // given
        var updateBody = new UpdateRequestDto(null, "새로운이름", null);
    ...
    void updateUserInfo_email_success() throws Exception {
        // given
        var updateBody = new UpdateRequestDto(null, null, "newemail@example.com");
    ...
    void updateUserInfo_multipleFields_success() throws Exception {
        // given
        var updateBody = new UpdateRequestDto("newuser", "새이름", "new@example.com");
    ...
    @Test
    @DisplayName("중복된 username으로 수정 실패")
    void updateUserInfo_duplicateUsername_fail() throws Exception {
        // given - 다른 사용자 등록
        var anotherUser = new RegisterRequestDto(
                "anotheruser", "Passw0rd!", "다른사용자", "another@example.com"
        );
        mockMvc.perform(
                post("/user/register")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json(anotherUser))
        ).andExpect(status().isOk());

        // when & then - 중복된 username으로 수정 시도
        var updateBody = new UpdateRequestDto("anotheruser", null, null);
        mockMvc.perform(
                put("/user/update")
                        .session(session)
                        .with(csrf())
    ...
    void updateUserInfo_emptyRequest_noChange() throws Exception {
        // given
        var updateBody = new UpdateRequestDto(null, null, null);
