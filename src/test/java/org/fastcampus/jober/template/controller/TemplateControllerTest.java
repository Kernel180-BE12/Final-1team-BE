...
//    Long spaceId = 1L;
//    Long templateId = 1L;
//    TemplateDetailResponseDto invalidTemplate =
//        TemplateDetailResponseDto.builder()
//            .id(1L)
...
//    // When & Then
//    mockMvc
//        .perform(get("/template/invalid").contentType(MediaType.APPLICATION_JSON))
...
//    // When & Then
//    mockMvc
//        .perform(get("/template/invalid/invalid").contentType(MediaType.APPLICATION_JSON))
//        .andExpect(status().isBadRequest()); // GlobalExceptionHandler가 처리하여 400으로 응답
//  }
//
...
//  }
//
//  /** 템플릿 삭제 실패 테스트 (템플릿 없음) 존재하지 않는 템플릿을 삭제하려고 할 때 404 에러가 발생하는지 검증 */
//  @Test
//  @WithMockUser
...
//        .deleteTemplate(any(TemplateDeleteRequestDto.class));
//
//    String deleteRequestJson =
//        """
//                {
...
//  }
//
//  /** 템플릿 삭제 실패 테스트 (권한 없음) 다른 스페이스의 템플릿을 삭제하려고 할 때 권한 에러가 발생하는지 검증 */
//  @Test
//  @WithMockUser
...
//                .content(deleteRequestJson))
//        .andExpect(
//            status().isInternalServerError()); // GlobalExceptionHandler가 IllegalArgumentException을
//    // 500으로 처리
//  }
//
//  /** 템플릿 삭제 실패 테스트 (이미 삭제된 템플릿) 이미 삭제된 템플릿을 다시 삭제하려고 할 때 에러가 발생하는지 검증 */
//  @Test
//  @WithMockUser
